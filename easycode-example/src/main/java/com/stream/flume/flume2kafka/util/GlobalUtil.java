 ...
 * under the License.
 */
package com.stream.flume.flume2kafka.util;
...
public class GlobalUtil {
    ...
    public GlobalUtil(boolean isUdp) {
        this(DEFAULT_SIZE, SyslogSourceConfigurationConstants.DEFAULT_KEEP_FIELDS, isUdp);
    }

    public GlobalUtil(Integer eventSize, boolean keepFields, boolean isUdp) {
        isBadEvent = false;
    ...
    public Event buildMessage(long time, byte[] reqBytes) {
        try {
			return EventBuilder.withBody(time + "|" + new String(reqBytes, "UTF-8"), Charset.defaultCharset());
